...
## 🚀 Zero-Overhead Telemetry & Performance

OffHeap-Forge는 메인 데이터 처리 파이프라인에 단 1나노초의 지연(Overhead)도 발생시키지 않는 **무결점 모니터링 아키텍처**를 구축했습니다.

### 🧠 관측자 효과(Observer Effect) 회피 설계
속도를 측정하기 위한 코드가 오히려 시스템을 느려지게 만드는 현상을 방지하기 위해, 오직 순수 자바 내장 클래스만을 사용하여 백그라운드 스레드에 메트릭 서버를 띄웠습니다. 
메인 데이터 파이프라인이 격렬하게 작동하는 동안, 제3의 관찰자 스레드가 락(Lock) 없이 인덱스만 읽기 전용으로 참조하여 Prometheus 규격의 데이터를 노출합니다.

### 📊 성능 스트레스 테스트 (Stress Test)
- **테스트 환경**: Windows PowerShell TCP Socket Client -> Localhost Java Engine
- **측정 결과**: **안정적인 67,000+ TPS (초당 처리량) 달성**
- **한계점 분석**: 67K TPS는 오프힙 엔진의 한계가 아닌, 트래픽을 발송하는 클라이언트(PowerShell)의 물리적 발송 한계치입니다. 엔진 자체의 단일 메시지 처리 속도는 **~5.7ns** 수준으로, 더 강력한 부하 테스트 도구 사용 시 수십만 TPS 이상의 압도적인 Throughput을 보장합니다.

## 🛠️ 실시간 시각화 대시보드 실행 방법 (Docker)
1. **엔진 구동**: java ForgeMain (포트 9999, 9090 개방)
2. **모니터링 스택 실행**: docker-compose up -d
3. **Grafana 접속**: http://localhost:3000 (admin/admin) 접속 후 대시보드 확인
